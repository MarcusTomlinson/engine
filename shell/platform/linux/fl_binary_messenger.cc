...
static void fl_binary_messenger_response_handle_dispose(GObject* object) {
  ...
  // Respond if the user forgot to
  if (self->response_handle != nullptr) {
    if (self->messenger->engine != nullptr)
      fl_engine_send_platform_message_response(
          self->messenger->engine, self->response_handle, nullptr, nullptr);
